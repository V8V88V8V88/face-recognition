...
void recognizeFaces(const vector<Mat>& knownFaceImages, const vector<string>& knownFaceNames) {
    ...
    if (!faceCascade.load(cv::samples::findFile("lbpcascade_frontalface.xml"))) {
    ...
    while (true) {
        ...
        for (const auto& face : faces) {
            ...
            // Convert the predicted label back to the corresponding person's name
            ...
            putText(frame, "Person: " + personName, Point(15, 40), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 255, 255), 2);
...
int main() {
    ...
    for (const auto& entry : fs::directory_iterator(faces_folder)) {
        ...
        string personName = entry.path().stem().string(); // Extracting name without extension
